...
export function calculateScores(
  ...
  for (const cn of chineseNews) {
    ...
    if (newsMap.has(key)) {
      ...
      newsMap.set(key, {
        id: cn.id,
        title: cn.title,
        titleZh: cn.title, // Chinese source already has Chinese title
        url: cn.url,
        source: [cn.source],
        hotness: 0,
  ...
  const results = Array.from(newsMap.values());
  ...
  for (const news of results) {
    ...
    // Mark if this is native Chinese content
    news.isTranslated = news.titleZh !== undefined && news.titleZh !== news.title;
  }
